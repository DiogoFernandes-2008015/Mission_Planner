...
def get_wind(z):
    """
    Perfil de vento usando a Lei de Potência.
    Assume que o vento é mais fraco no solo e aumenta com a altitude.
    """
    z_ref = 10.0       # Altura de referência (m)
    alpha = 0.143      # Expoente típico para terreno aberto/plano
    
    # Velocidade do vento na altura de referência
    wind_x_ref = 50.0
    wind_y_ref = 50.0
    wind_z_ref = 0.0   # O vento vertical é geralmente negligenciável
    
    # Evita valores negativos ou zero na base da potência
    z_clip = jnp.maximum(z, 0.001) 
    
    # Fator de escala do vento com base na altitude
    fator = (z_clip / z_ref) ** alpha
    
    # Zera o vento se estiver abaixo ou exatamente no solo
    fator = jnp.where(z > 0.0, fator, 0.0)
    
    return jnp.array([wind_x_ref * fator, wind_y_ref * fator, wind_z_ref])

def eqdif(t, state, args):
    x, y, z, vx, vy, vz = state
    v = jnp.array([vx, vy, vz])
    
    # Adiciona a influência do vento
    v_wind = get_wind(z)
    v_rel = v - v_wind
    v_rel_norm = jnp.linalg.norm(v_rel)
    
    # Cálculos aerodinâmicos com base na velocidade relativa
    mach = v_rel_norm / 340.0
    rho = get_atmosphere(z)
    Cd = get_cd(mach)
    
    P_grav = jnp.array([0.0, 0.0, -m * g])
    Drag = -0.5 * rho * S * Cd * v_rel_norm * v_rel
...
def executar_pso_balistica(n, p, alfa, beta, nmi, tol, vmin, linf, lsup, xd, yd, z0, k=1, count=0):
    ...
    line, = ax.plot([], [], 'b-')
    ax.grid(True)
    ax.set_title("Otimização de Lançamento (PSO + Diffrax c/ Vento)")
